...
const styles = (theme: Theme) => ({
  portal: rule({
    ...
    bottom: -1,
    left: -1,
    zIndex: styledBy<IProps, 'type', number>('type', {
  ...
  overlay: rule({
    ...
    backgroundColor: theme.extra.palette.bg.overlay,
  }),
  modal: rule({
    position: 'relative',
    flexGrow: 1,
    ...
    [theme.breakpoints.up('sm')]: rule({
      ...
      minHeight: 'unset',
      minWidth: 'unset',
      width: styledBy<IProps, 'size', string>('size', {
        small: '16rem',
        medium: '25rem',
        large: '35rem',
        xLarge: '45rem',
  ...
  isAbsolute: {},
  isHidden: {
    display: styledBy<IProps, 'titleAlign', string>('titleAlign', {
      center: 'block',
      left: 'none',
    }, 'center'),
  },
